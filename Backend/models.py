...
class End_User(EndUserBase, table=True):
    ...
    image_url_id: int | None = Field(default=1, foreign_key="image_url.image_url_id")
    transfer_history_id: int | None = Field(default=None,
                                            foreign_key="transfer_history.transfer_history_id")
    password_hash: str
...
class Custom_Shelf_Book_Link(SQLModel, table=True):
    custom_shelf_id: Optional[int] = Field(default=None,
                                           foreign_key="custom_shelf.shelf_id",
                                           primary_key=True)
    bookshelf_id: Optional[int] = Field(default=None,
                                        foreign_key="read_shelf_book.bookshelf_id",
                                        primary_key=True)
...
class Custom_Shelf(CustomShelfBase, table=True):
    ...
    shelf_books: List["Read_Shelf_Book"] = Relationship(back_populates="custom_shelves",
...
class Book(SQLModel, table=True):
    ...
    title: str
    authors: List[str] | None = Field(sa_column=Column(ARRAY(String)),
                                      default_factory=list)
    description: str
    number_of_pages: int
    categories: List[str] | None = Field(sa_column=Column(ARRAY(String)),
                                         default_factory=list)
...
class Read_Shelf_Book(SQLModel, table=True):
    ...
    reading_goal_id: int | None = Field(default=None,
                                        foreign_key="reading_goal.reading_goal_id")
    date_read: datetime | None
    rating: float | None
    __table_args__ = (
        UniqueConstraint("book_id", "read_shelf_id", name="unique_read_shelf_book"),)


class To_Read_Shelf_Book(SQLModel, table=True):
    bookshelf_id: int | None = Field(default=None, primary_key=True)
    to_read_shelf_id: int | None = Field(default=None,
                                         foreign_key="to_read_shelf.shelf_id")
    book_id: int | None = Field(default=None, foreign_key="book.book_id")
...
class Log_Section(SQLModel, table=True):
    ...
    journal_entry_id: int | None = Field(default=None,
                                         foreign_key="journal_entry.journal_entry_id",
                                         ondelete="CASCADE")
    section_name: str
    entry_text: str
